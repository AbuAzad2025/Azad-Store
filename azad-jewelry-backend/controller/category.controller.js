...
const maybeDownloadCategoryImageToLocal = async ({ imageUrl, productType, parent }) => {
  const raw = typeof imageUrl === "string" ? imageUrl.trim() : "";
  if (!raw) return imageUrl;
  if (isLocalCategoryImagePath(raw)) return raw;
  ...
  const { rel, abs } = buildLocalCategoryImagePath({
    productType,
    parent,
    baseName: `${parent || "category"}-image-${hash}`,
  });
...
exports.addCategory = async (req,res,next) => {
  try {
    if (req?.body?.img) {
      req.body.img = await maybeDownloadCategoryImageToLocal({
