...
const BestSellerPrd = () => {
  const { data: products, isError, isLoading } =
    useGetProductTypeQuery({ type: 'jewelry', query: `topSellers=true` });
  const { t } = useLanguage();
            ...
            <div className="row">
              <div className="col-xl-12">
                  <div className="tp-section-title-wrapper-4 mb-50 text-center">
                    <span className="tp-section-title-pre-4">{t("bestSellerThisWeek")}</span>
                    <h3 className="tp-section-title-4">{t("topSellersForYou")}</h3>
                  </div>
              </div>
            </div>
            <div className="row">
