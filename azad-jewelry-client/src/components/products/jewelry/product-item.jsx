...
const ProductItem = ({ product }) => {
  ...
  const primaryTag = Array.isArray(tags) ? tags[0] : "";
  const imageSrc = img || product?.imageURLs?.[0]?.img || "/assets/img/product/category/product-cat-1.png";

  return (
    <div className="tp-product-item-4 p-relative mb-40">
      <div className="tp-product-thumb-4 p-relative fix">
        <Link href={`/product-details/${_id}`}>
          <Image src={imageSrc} alt="product img" width={284} height={352} />
        </Link>
        <div className="tp-product-badge">
          {status === 'out-of-stock' && <span className="product-hot">{t("outOfStock")}</span>}
        </div>
        <div className="tp-product-action-3 tp-product-action-4 has-shadow tp-product-action-blackStyle tp-product-action-brownStyle">
          <div className="tp-product-action-item-3 d-flex flex-column">
            {isAddedToCart ? (
              ...
              >
                <Cart />
                <span className="tp-product-tooltip">{t("viewCart")}</span>
              </Link>
            ) : (
              <button
                type="button"
