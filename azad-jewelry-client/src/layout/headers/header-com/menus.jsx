...
const Menus = () => {
  ...
  if (!isLoading && !isError && products?.data?.length === 0) {
    content = <ErrorMsg msg={t("noProducts")} />;
  }
      ...
      {menu_data.map((menu) =>
        menu.homes ? (
          <li key={menu.id} className="has-dropdown has-mega-menu">
            <Link href={menu.link}>{language === "ar" ? menu.title : menu.title_en}</Link>
              ...
              {menu.product_pages.map((p, i) => (
                <li key={i} className="has-dropdown">
                  <Link href={p.link} className="mega-menu-title">
                    {language === "ar" ? p.title : p.title_en}
                  </Link>
                  <ul className="tp-submenu">
                    {p.mega_menus.map((m, i) => (
                      <li key={i}>
                        <Link href={m.link}>{language === "ar" ? m.title : m.title_en}</Link>
                      </li>
                    ))}
                  </ul>
                </li>
          ...
          <li key={menu.id} className="has-dropdown">
            ...
            <ul className="tp-submenu">
              {menu.sub_menus.map((b, i) => (
                <li key={i}>
                  <Link href={b.link}>{language === "ar" ? b.title : b.title_en}</Link>
                </li>
              ))}
            </ul>
          </li>
