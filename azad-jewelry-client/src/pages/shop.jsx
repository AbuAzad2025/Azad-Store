...
const ShopPage = ({ query }) => {
  ...
  if (!isLoading && isError) {
    content = <div className="pb-80 text-center"><ErrorMsg msg="errorGeneric" /></div>;
  }
  if (!isLoading && !isError && products?.data?.length === 0) {
  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    ...
    if (!selectValue || selectValue === "Default Sorting") {
  ...
  return (
    <Wrapper>
      <SEO pageTitle={t("shop")} />
      <HeaderTwo style_2={true} />
      <ShopBreadcrumb title={t("shopGrid")} subtitle={t("shopGrid")} />
      {content}
